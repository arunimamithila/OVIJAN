...
function PostCard({ post, delay = 0, savedIds, onToggleSave }) {
  ...
  return (
    <div className="post-card" style={{ background: "#fff", borderRadius: 18, border: "1px solid rgba(0,0,0,0.07)", boxShadow: "0 2px 16px rgba(0,0,0,0.07)", marginBottom: 14, overflow: "hidden", animation: `fade-up .4s ease ${delay}ms both`, display: "flex", flexDirection: "column" }}>
      {/* Square image */}
      {post.hasImage && (
        <div style={{ width: "100%", aspectRatio: "16/9", overflow: "hidden", flexShrink: 0 }}>
          <img src={post.imgUrl} alt="" style={{ width: "100%", height: "100%", objectFit: "cover", display: "block" }} onError={e => { e.target.parentElement.style.display = "none"; }} />
        </div>
      )}
      <div style={{ padding: "18px 20px 14px", flex: 1, display: "flex", flexDirection: "column" }}>
        ...
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 12 }}>
          <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
            <div style={{ width: 36, height: 36, borderRadius: "50%", background: `linear-gradient(135deg, ${G.mint}, #1aad51)`, display: "flex", alignItems: "center", justifyContent: "center", color: "#fff", fontWeight: 800, fontSize: 12, fontFamily: FONT, flexShrink: 0, boxShadow: "0 2px 8px rgba(35,197,94,0.3)" }}>{post.avatar}</div>
            <div>
              <div style={{ fontWeight: 700, fontSize: 13, color: "#111827", fontFamily: FONT }}>{post.author}</div>
              <div style={{ color: "#9ca3af", fontSize: 11, fontFamily: FONT }}>{post.time}</div>
            </div>
          </div>
          <div style={{ display: "flex", alignItems: "center", gap: 6 }}>
            <span style={{ padding: "3px 10px", borderRadius: 99, background: post.categoryColor, color: post.categoryText, fontSize: 10, fontWeight: 700, fontFamily: FONT }}>⚡ {post.category}</span>
            {/* Bookmark button */}
            <button onClick={() => onToggleSave(post.id)} title={isSaved ? "Remove from saved" : "Save post"} style={{ background: isSaved ? "rgba(251,191,36,0.1)" : "rgba(0,0,0,0.04)", border: `1px solid ${isSaved ? "rgba(251,191,36,0.3)" : "rgba(0,0,0,0.08)"}`, borderRadius: 8, width: 30, height: 30, display: "flex", alignItems: "center", justifyContent: "center", cursor: "pointer", color: isSaved ? G.gold : "#9ca3af", transition: "all .15s" }}>
              <svg width="13" height="13" viewBox="0 0 24 24" fill={isSaved ? G.gold : "none"} stroke="currentColor" strokeWidth="2.2" strokeLinecap="round"><path d="M19 21l-7-5-7 5V5a2 2 0 0 1 2-2h10a2 2 0 0 1 2 2z"/></svg>
            </button>
          </div>
        </div>
        {/* Title + excerpt */}
        <h3 style={{ fontWeight: 800, fontSize: 15, color: "#111827", fontFamily: FONT, lineHeight: 1.4, marginBottom: 7, flex: 0 }}>{post.title}</h3>
        <p style={{ color: "#6b7280", fontSize: 13, fontFamily: FONT, lineHeight: 1.6, marginBottom: 12, flex: 1 }}>{post.excerpt}</p>
        {/* Tags */}
        <div style={{ display: "flex", gap: 6, flexWrap: "wrap", marginBottom: 14 }}>
          {post.tags.map(tag => <span key={tag} style={{ padding: "3px 10px", borderRadius: 99, fontSize: 10, fontWeight: 600, background: "rgba(35,197,94,0.08)", color: "#23c55e", border: "1px solid rgba(35,197,94,0.2)" }}>#{tag}</span>)}
        </div>
      </div>
      {/* Action bar */}
      <div style={{ padding: "10px 20px 14px", borderTop: "1px solid rgba(0,0,0,0.05)", display: "flex", alignItems: "center", gap: 2 }}>
        <button className="action-btn" onClick={() => setLiked(l => !l)} style={{ color: liked ? "#ef4444" : "#6b7280" }}>
          <svg width="15" height="15" viewBox="0 0 24 24" fill={liked ? "#ef4444" : "none"} stroke={liked ? "#ef4444" : "currentColor"} strokeWidth="2" strokeLinecap="round"><path d="M20.84 4.61a5.5 5.5 0 0 0-7.78 0L12 5.67l-1.06-1.06a5.5 5.5 0 0 0-7.78 7.78l1.06 1.06L12 21.23l7.78-7.78 1.06-1.06a5.5 5.5 0 0 0 0-7.78z"/></svg>
...
// ── PostView with independent scroll + working Saved tab ────────────────────
function PostView() {
  ...
  const visiblePosts = activeTab === "Saved Posts"
    ? POSTS.filter(p => savedIds.has(p.id))
    : POSTS;

  // Sidebar communities — joined ones from current tab
  const sidebarComms = ALL_COMMUNITIES[commTab].filter(c => {
    const joinedLocally = ALL_COMMUNITIES[commTab].find(x => x.name === c.name);
    return joinedLocally?.joined;
  }).slice(0, 4);

  // Use all joined across all tabs for the sidebar count
  ...
  return (
    <>
      ...
      {/* Outer wrapper fills viewport height minus topbar — uses flex so columns manage their own scroll */}
      <div style={{ display: "grid", gridTemplateColumns: "1fr 300px", gap: 20, animation: "fade-up .4s ease both", height: "calc(100vh - 110px)", overflow: "hidden" }}>

        {/* ── Left: independently scrolling post feed ── */}
        <div style={{ display: "flex", flexDirection: "column", overflow: "hidden" }}>
          ...
          <div style={{ flexShrink: 0, paddingBottom: 0 }}>
            ...
            <div style={{ display: "flex", borderBottom: "2px solid rgba(255,255,255,0.06)", marginBottom: 18 }}>
              {["All Posts", "Saved Posts"].map(tab => (
                <button key={tab} onClick={() => setActiveTab(tab)} style={{ padding: "8px 20px", borderRadius: 0, border: "none", background: "transparent", cursor: "pointer", fontFamily: FONT, fontSize: 13, fontWeight: activeTab === tab ? 700 : 500, color: activeTab === tab ? G.text : G.textSub, borderBottom: activeTab === tab ? `2px solid ${G.mint}` : "2px solid transparent", marginBottom: -2, transition: "all .15s", display: "flex", alignItems: "center", gap: 7 }}>
                  {tab}
                  {tab === "Saved Posts" && savedIds.size > 0 && (
                    <span style={{ padding: "1px 7px", borderRadius: 99, background: "rgba(251,191,36,0.15)", border: "1px solid rgba(251,191,36,0.3)", color: G.gold, fontSize: 10, fontWeight: 800, fontFamily: FONT }}>{savedIds.size}</span>
                  )}
                </button>
              ))}
            </div>
          ...
          <div style={{ flex: 1, overflowY: "auto", paddingRight: 4 }}>
            {visiblePosts.length === 0 ? (
              <div style={{ textAlign: "center", padding: "60px 20px" }}>
                ...
                <div style={{ fontWeight: 700, fontSize: 16, color: G.text, fontFamily: FONT, marginBottom: 6 }}>No saved posts yet</div>
                <div style={{ color: G.textSub, fontSize: 13, fontFamily: FONT }}>Bookmark posts using the 🔖 icon to find them here.</div>
              </div>
            ) : (
              visiblePosts.map((p, i) => (
                <PostCard key={p.id} post={p} delay={i * 60} savedIds={savedIds} onToggleSave={toggleSave} />
              ))
            )}
          </div>
        </div>

        {/* ── Right: independently scrolling sidebar ── */}
        <div style={{ overflowY: "auto", display: "flex", flexDirection: "column", gap: 14 }}>

          {/* Communities card */}
          <div style={{ background: "#1a2537", borderRadius: 18, padding: 20, border: "1px solid rgba(255,255,255,0.07)", animation: "fade-up .4s ease .1s both", flexShrink: 0 }}>
            <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 4 }}>
              <div style={{ fontWeight: 800, fontSize: 15, color: G.text, fontFamily: FONT }}>My Communities</div>
              <span style={{ padding: "2px 8px", borderRadius: 99, background: "rgba(35,197,94,0.1)", border: "1px solid rgba(35,197,94,0.22)", color: G.mint, fontSize: 10, fontWeight: 800, fontFamily: FONT }}>{joinedAll.length} joined</span>
            </div>
            <div style={{ color: G.textSub, fontSize: 11, fontFamily: FONT, marginBottom: 14 }}>Filter by type</div>

            {/* Mini tab for community type */}
            <div style={{ display: "flex", gap: 3, background: "rgba(255,255,255,0.04)", borderRadius: 10, padding: 3, marginBottom: 14, border: "1px solid rgba(255,255,255,0.06)" }}>
              {[{ id: "country", label: "Countries" }, { id: "city", label: "Cities" }, { id: "university", label: "Unis" }].map(t => (
                <button key={t.id} onClick={() => setCommTab(t.id)} style={{ flex: 1, padding: "5px 4px", borderRadius: 7, border: "none", background: commTab === t.id ? "rgba(35,197,94,0.15)" : "transparent", color: commTab === t.id ? G.mint : G.textSub, fontWeight: commTab === t.id ? 700 : 500, fontSize: 10, fontFamily: FONT, cursor: "pointer", transition: "all .15s" }}>{t.label}</button>
              ))}
            </div>

            {ALL_COMMUNITIES[commTab].filter(c => c.joined).map(c => (
              <div key={c.name} className="comm-row" style={{ display: "flex", alignItems: "center", gap: 11, marginBottom: 3 }}>
                <div style={{ width: 38, height: 38, borderRadius: 11, background: c.bg, display: "flex", alignItems: "center", justifyContent: "center", fontSize: 17, flexShrink: 0 }}>{c.emoji}</div>
                <div style={{ flex: 1, minWidth: 0 }}>
                  <div style={{ fontWeight: 700, fontSize: 12, color: G.text, fontFamily: FONT, whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis" }}>{c.name}</div>
                  <div style={{ color: G.textSub, fontSize: 10, fontFamily: FONT }}>{c.members} Members</div>
                </div>
                <div style={{ width: 6, height: 6, borderRadius: "50%", background: G.mint, flexShrink: 0 }} />
              </div>
            ))}
            {ALL_COMMUNITIES[commTab].filter(c => c.joined).length === 0 && (
              <div style={{ color: G.textMuted, fontSize: 12, fontFamily: FONT, padding: "8px 0" }}>No joined communities in this category.</div>
            )}

            <button onClick={() => setShowAllComm(true)} style={{ marginTop: 12, width: "100%", padding: "8px 0", background: "rgba(35,197,94,0.06)", border: "1px solid rgba(35,197,94,0.18)", borderRadius: 10, color: G.mint, fontWeight: 700, fontSize: 12, fontFamily: FONT, cursor: "pointer", transition: "all .15s", display: "flex", alignItems: "center", justifyContent: "center", gap: 6 }}
          ...
          {/* Trending card */}
          <div style={{ background: "#1a2537", borderRadius: 18, padding: 20, border: "1px solid rgba(255,255,255,0.07)", animation: "fade-up .4s ease .18s both", flexShrink: 0 }}>
