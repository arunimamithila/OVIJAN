...
if (typeof document !== "undefined") {
  ...
  style.textContent = `
    ...
    /* Standard fixed overlay — used by Edit Profile, Visa full-screen, EditModal, SkillsModal */
    .overlay-bg{
      position:fixed;
      inset:0;
    ...
    /* ── Main-content-scoped overlay — used by AllJobsModal AND AllRequirementsModal ── */
    /* Positioned absolute so it covers only the dashboard area,
       not the left sidebar. The parent must be position:relative. */
    .overlay-main-area{
      position:absolute;
      inset:0;
...
// ─────────────────────────────────────────────────────────────────────────────
// AllRequirementsModal — now uses overlay-main-area (position:absolute) so it
// is scoped to the main content column. Rendered by WorkView as a sibling to
// the dashboard content, NOT inside VisaRequirementsPanel.
// ─────────────────────────────────────────────────────────────────────────────
...
// ─────────────────────────────────────────────────────────────────────────────
// VisaRequirementsPanel — no longer owns AllRequirementsModal state.
// Calls onShowAllRequirements(country) prop so WorkView can render the modal
// at the correct level (scoped to the main content area, not inside this card).
// ─────────────────────────────────────────────────────────────────────────────
function VisaRequirementsPanel({ onShowAllRequirements }) {
  ...
  return (
    <div style={{ position: "relative" }}>
      ...
      {/* Calls the lifted handler — no local modal state */}
      <button
        onClick={() => onShowAllRequirements(country)}
        style={{ background: "none", border: "none", color: G.mint, fontWeight: 700, fontSize: 13, fontFamily: FONT, cursor: "pointer", display: "flex", alignItems: "center", gap: 6, padding: 0 }}
...
function RecommendedJobsPanel({ onShowAllJobs }) {
  const jobs = [
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", height: "100%" }}>
      ...
      <button
        ...
        style={{ width: "100%", marginTop: 14, padding: "11px", borderRadius: 12, border: "1px solid rgba(35,197,94,0.2)", background: "rgba(35,197,94,0.06)", color: G.mint, fontWeight: 700, fontSize: 13, fontFamily: FONT, cursor: "pointer", transition: "all .15s", flexShrink: 0 }}
...
// WorkRightPanel passes both onShowAllJobs and onShowAllRequirements down to the tab panels
function WorkRightPanel({ onShowAllJobs, onShowAllRequirements }) {
  ...
  return (
    <div style={{ ...whiteCard(), position: "relative", overflow: "visible", animation: "fade-up .5s ease .3s both" }}>
      ...
      <div style={{ padding: "36px 28px 28px" }}>
        {activeTab === "Visa" && <VisaRequirementsPanel onShowAllRequirements={onShowAllRequirements} />}
        {activeTab === "Documents" && <WorkDocumentsPanel />}
        {activeTab === "Jobs" && <RecommendedJobsPanel onShowAllJobs={onShowAllJobs} />}
      </div>
    </div>
  );
}
...
// ─────────────────────────────────────────────────────────────────────────────
// WorkView — owns BOTH showAllJobs AND showRequirements state.
// Both AllJobsModal and AllRequirementsModal are rendered here as siblings to
// the dashboard content, inside the position:relative main-content wrapper.
// This ensures both modals use overlay-main-area (position:absolute) and are
// perfectly centered within the main content column, never over the sidebar.
// ─────────────────────────────────────────────────────────────────────────────
function WorkView({ profile }) {
  const [showAllJobs, setShowAllJobs] = useState(false);
  const [requirementsCountry, setRequirementsCountry] = useState(null); // null = closed

  const handleShowRequirements = (country) => setRequirementsCountry(country);
  const handleCloseRequirements = () => setRequirementsCountry(null);

  return (
    // position:relative — stacking context for both absolute overlays
    <div style={{ position: "relative" }}>

      {/* ── AllJobsModal overlay — covers only this column ── */}
      {showAllJobs && (
        <AllJobsModal onClose={() => setShowAllJobs(false)} />
      )}

      {/* ── AllRequirementsModal overlay — covers only this column ── */}
      {requirementsCountry && (
        <AllRequirementsModal
          country={requirementsCountry}
      ...
      <div style={{ paddingTop: 24 }}>
        {/* Pass both handlers down through WorkRightPanel to the correct tab panels */}
        <WorkRightPanel
          onShowAllJobs={() => setShowAllJobs(true)}
          onShowAllRequirements={handleShowRequirements}
        />
      </div>
    </div>
  );
...
// ── Author is always Novera Ahmed ──────────────────────────────────────────
const ME = { name: "Novera Ahmed", avatar: "NA" };

const POSTS = [
  { id: 1, author: ME.name, avatar: ME.avatar, time: "2 days ago", category: "Experience", categoryColor: "rgba(35,197,94,0.12)", categoryText: "#23c55e", title: "My journey of getting admission in Canada 🇨🇦", excerpt: "I wanted to share my complete journey of getting admission in a Canadian university. From choosing the right program to preparing documents and SOP writing, every step taught me something new.", tags: ["Canada", "Admission", "Study Abroad"], likes: 128, comments: 36, saved: false, hasImage: true, imgUrl: "https://images.unsplash.com/photo-1517935706615-2717063c2225?w=600&q=80" },
  { id: 2, author: ME.name, avatar: ME.avatar, time: "1 week ago", category: "Question", categoryColor: "rgba(250,191,36,0.12)", categoryText: "#f59e0b", title: "Which country is best for MS in Computer Science?", excerpt: "I'm confused between Canada, UK and Australia for MS in CS. Looking for good universities, PR opportunities and part-time work options. Any advice from people who've been through this?", tags: ["MS in CS", "Canada", "UK", "Australia"], likes: 87, comments: 64, saved: true, hasImage: false },
  { id: 3, author: ME.name, avatar: ME.avatar, time: "2 weeks ago", category: "Scholarship", categoryColor: "rgba(167,139,250,0.12)", categoryText: "#a78bfa", title: "Finally received my scholarship! 🎉", excerpt: "Thrilled to share that I've been awarded a 50% scholarship for my Master's program. Hard work really pays off — here's exactly what I did to make my application stand out.", tags: ["Scholarship", "Funding", "Masters"], likes: 142, comments: 52, saved: true, hasImage: true, imgUrl: "https://images.unsplash.com/photo-1579621970563-ebec7560ff3e?w=600&q=80" },
  { id: 4, author: ME.name, avatar: ME.avatar, time: "3 weeks ago", category: "Visa", categoryColor: "rgba(56,189,248,0.12)", categoryText: "#38bdf8", title: "Canadian Student Visa approved in 3 weeks ✅", excerpt: "Sharing my student visa timeline and the exact documents I submitted. The process was smoother than expected — biometrics done, medical cleared, and approval in record time.", tags: ["Visa", "Canada", "Student Visa"], likes: 203, comments: 91, saved: false, hasImage: false },
];

// ── Community data: country / city / university ─────────────────────────────
const ALL_COMMUNITIES = {
  country: [
    { name: "Canada 🇨🇦", members: "24.5K", emoji: "🇨🇦", bg: "rgba(35,197,94,0.1)", joined: true },
    { name: "Australia 🇦🇺", members: "18.7K", emoji: "🇦🇺", bg: "rgba(251,191,36,0.1)", joined: false },
    { name: "United Kingdom 🇬🇧", members: "15.2K", emoji: "🇬🇧", bg: "rgba(56,189,248,0.1)", joined: true },
    { name: "Germany 🇩🇪", members: "11.3K", emoji: "🇩🇪", bg: "rgba(167,139,250,0.1)", joined: false },
    { name: "USA 🇺🇸", members: "9.8K", emoji: "🇺🇸", bg: "rgba(248,113,113,0.1)", joined: false },
    { name: "Netherlands 🇳🇱", members: "6.1K", emoji: "🇳🇱", bg: "rgba(251,191,36,0.08)", joined: false },
  ],
  city: [
    { name: "Toronto", members: "9.2K", emoji: "🏙️", bg: "rgba(35,197,94,0.1)", joined: true },
    { name: "Vancouver", members: "7.8K", emoji: "🌊", bg: "rgba(56,189,248,0.1)", joined: false },
    { name: "London", members: "11.1K", emoji: "🎡", bg: "rgba(167,139,250,0.1)", joined: true },
    { name: "Berlin", members: "5.4K", emoji: "🏛️", bg: "rgba(251,191,36,0.1)", joined: false },
    { name: "Sydney", members: "6.6K", emoji: "🦘", bg: "rgba(248,113,113,0.1)", joined: false },
    { name: "Montreal", members: "4.9K", emoji: "🍁", bg: "rgba(99,102,241,0.1)", joined: false },
  ],
  university: [
    { name: "University of Toronto", members: "3.8K", emoji: "🎓", bg: "rgba(35,197,94,0.1)", joined: true },
    { name: "UBC Vancouver", members: "3.1K", emoji: "🎓", bg: "rgba(56,189,248,0.1)", joined: false },
    { name: "McGill University", members: "2.6K", emoji: "🎓", bg: "rgba(167,139,250,0.1)", joined: true },
    { name: "TU Berlin", members: "2.2K", emoji: "🎓", bg: "rgba(251,191,36,0.1)", joined: false },
    { name: "University of Melbourne", members: "2.9K", emoji: "🎓", bg: "rgba(248,113,113,0.1)", joined: false },
    { name: "Imperial College London", members: "2.4K", emoji: "🎓", bg: "rgba(99,102,241,0.1)", joined: false },
  ],
};
...
function AllCommunitiesModal({ onClose }) {
  const [tab, setTab] = useState("country");
  const [joined, setJoined] = useState(() => {
    const init = {};
    Object.values(ALL_COMMUNITIES).flat().forEach(c => { if (c.joined) init[c.name] = true; });
    return init;
  });
  const tabs = [{ id: "country", label: "Countries", icon: "🌍" }, { id: "city", label: "Cities", icon: "🏙️" }, { id: "university", label: "Universities", icon: "🎓" }];
  const list = ALL_COMMUNITIES[tab] || [];
  return (
    <div className="overlay-bg" onClick={e => e.target === e.currentTarget && onClose()}>
      <div style={{ background: "#0e1828", border: "1px solid rgba(255,255,255,0.09)", borderRadius: 28, width: 620, maxWidth: "94vw", maxHeight: "88vh", overflow: "hidden", display: "flex", flexDirection: "column", boxShadow: "0 48px 120px rgba(0,0,0,0.8)", animation: "modal-in .35s cubic-bezier(.22,1,.36,1) both" }}>
        {/* Header */}
        <div style={{ padding: "26px 28px 0", flexShrink: 0 }}>
          <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 18 }}>
            <div>
              <div style={{ fontWeight: 900, fontSize: 20, color: G.text, fontFamily: FONT, letterSpacing: -.5 }}>All Communities</div>
              <div style={{ color: G.textSub, fontSize: 12, fontFamily: FONT, marginTop: 3 }}>Discover and join communities worldwide</div>
            </div>
            <button onClick={onClose} style={{ width: 36, height: 36, borderRadius: 10, background: "rgba(255,255,255,0.05)", border: "1px solid rgba(255,255,255,0.09)", color: G.textSub, cursor: "pointer", fontSize: 20, display: "flex", alignItems: "center", justifyContent: "center" }}>×</button>
          </div>
          {/* Tab switcher */}
          <div style={{ display: "flex", gap: 4, background: "rgba(255,255,255,0.04)", borderRadius: 14, padding: 4, border: "1px solid rgba(255,255,255,0.07)", marginBottom: 0 }}>
            {tabs.map(t => (
              <button key={t.id} onClick={() => setTab(t.id)} style={{ flex: 1, padding: "9px 12px", borderRadius: 10, border: tab === t.id ? "1px solid rgba(255,255,255,0.1)" : "1px solid transparent", background: tab === t.id ? "rgba(255,255,255,0.08)" : "transparent", color: tab === t.id ? G.text : G.textSub, fontWeight: tab === t.id ? 700 : 500, fontSize: 13, fontFamily: FONT, cursor: "pointer", transition: "all .2s", display: "flex", alignItems: "center", justifyContent: "center", gap: 6 }}>
        ...
        <div style={{ flex: 1, overflowY: "auto", padding: "16px 28px 24px" }}>
          <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
            {list.map((c, i) => (
              <div key={c.name} style={{ display: "flex", alignItems: "center", gap: 14, padding: "14px 16px", borderRadius: 16, background: joined[c.name] ? "rgba(35,197,94,0.05)" : "rgba(255,255,255,0.02)", border: `1px solid ${joined[c.name] ? "rgba(35,197,94,0.2)" : "rgba(255,255,255,0.06)"}`, animation: `fade-up .3s ease ${i * 40}ms both`, transition: "all .2s" }}>
                <div style={{ width: 46, height: 46, borderRadius: 13, background: c.bg, display: "flex", alignItems: "center", justifyContent: "center", fontSize: 22, flexShrink: 0 }}>{c.emoji}</div>
                <div style={{ flex: 1 }}>
                  <div style={{ fontWeight: 700, fontSize: 14, color: G.text, fontFamily: FONT }}>{c.name}</div>
                  <div style={{ color: G.textSub, fontSize: 11, fontFamily: FONT, marginTop: 2 }}>{c.members} members</div>
                </div>
                <button onClick={() => setJoined(j => ({ ...j, [c.name]: !j[c.name] }))} style={{ padding: "7px 18px", borderRadius: 10, border: `1px solid ${joined[c.name] ? "rgba(35,197,94,0.35)" : "rgba(255,255,255,0.12)"}`, background: joined[c.name] ? "rgba(35,197,94,0.12)" : "rgba(255,255,255,0.04)", color: joined[c.name] ? G.mint : G.textSub, fontWeight: 700, fontSize: 12, fontFamily: FONT, cursor: "pointer", transition: "all .15s", flexShrink: 0 }}>
...
// ── True-square PostCard (1:1 aspect ratio, image fills top half) ────────────
function PostCard({ post, delay = 0, savedIds, onToggleSave }) {
  ...
  return (
    <div className="post-card" style={{
      ...
      overflow: "hidden",
      animation: `fade-up .35s ease ${delay}ms both`,
      display: "flex",
      flexDirection: "column",
      aspectRatio: "1 / 1",   /* ← true square */
    }}>

      {/* Top half: image OR gradient placeholder */}
      <div style={{ flex: "0 0 45%", position: "relative", overflow: "hidden", background: post.hasImage ? "#000" : `linear-gradient(135deg, ${post.categoryColor.replace("0.12","0.25")} 0%, rgba(11,17,32,0.9) 100%)` }}>
        {post.hasImage
          ? <img src={post.imgUrl} alt="" style={{ width: "100%", height: "100%", objectFit: "cover", display: "block", opacity: 0.92 }} onError={e => { e.target.parentElement.style.background = `linear-gradient(135deg,rgba(35,197,94,0.2),rgba(11,17,32,0.9))`; e.target.style.display="none"; }} />
          : <div style={{ width: "100%", height: "100%", display: "flex", alignItems: "center", justifyContent: "center" }}>
              <span style={{ fontSize: 36, opacity: 0.6 }}>{post.category === "Question" ? "❓" : post.category === "Scholarship" ? "🎓" : post.category === "Visa" ? "🛂" : "💡"}</span>
            </div>
        }
        {/* Category badge overlay */}
        <div style={{ position: "absolute", top: 9, left: 9 }}>
          <span style={{ padding: "3px 9px", borderRadius: 99, background: post.categoryColor, color: post.categoryText, fontSize: 9, fontWeight: 800, fontFamily: FONT, backdropFilter: "blur(6px)" }}>⚡ {post.category}</span>
        </div>
        {/* Bookmark overlay */}
        <button onClick={() => onToggleSave(post.id)} style={{ position: "absolute", top: 8, right: 8, width: 26, height: 26, borderRadius: 7, background: isSaved ? "rgba(251,191,36,0.9)" : "rgba(255,255,255,0.85)", border: "none", display: "flex", alignItems: "center", justifyContent: "center", cursor: "pointer", color: isSaved ? "#fff" : "#9ca3af", boxShadow: "0 2px 8px rgba(0,0,0,0.15)", transition: "all .15s" }}>
          <svg width="11" height="11" viewBox="0 0 24 24" fill={isSaved ? "currentColor" : "none"} stroke="currentColor" strokeWidth="2.5" strokeLinecap="round"><path d="M19 21l-7-5-7 5V5a2 2 0 0 1 2-2h10a2 2 0 0 1 2 2z"/></svg>
        </button>
        {/* Gradient fade into card body */}
        <div style={{ position: "absolute", bottom: 0, left: 0, right: 0, height: 24, background: "linear-gradient(to bottom, transparent, #fff)" }} />
      </div>

      {/* Bottom half: content */}
      <div style={{ flex: 1, padding: "10px 13px 0", display: "flex", flexDirection: "column", overflow: "hidden", minHeight: 0 }}>
        {/* Author */}
        <div style={{ display: "flex", alignItems: "center", gap: 7, marginBottom: 7, flexShrink: 0 }}>
          <div style={{ width: 24, height: 24, borderRadius: "50%", background: `linear-gradient(135deg,${G.mint},#1aad51)`, display: "flex", alignItems: "center", justifyContent: "center", color: "#fff", fontWeight: 800, fontSize: 9, fontFamily: FONT, flexShrink: 0 }}>{post.avatar}</div>
          <div style={{ flex: 1, minWidth: 0 }}>
            <div style={{ fontWeight: 700, fontSize: 11, color: "#111827", fontFamily: FONT, whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis" }}>{post.author}</div>
            <div style={{ color: "#9ca3af", fontSize: 9, fontFamily: FONT }}>{post.time}</div>
          </div>
        </div>

        {/* Title — 2 lines max */}
        <h3 style={{ fontWeight: 800, fontSize: 12, color: "#111827", fontFamily: FONT, lineHeight: 1.4, margin: "0 0 5px", display: "-webkit-box", WebkitLineClamp: 2, WebkitBoxOrient: "vertical", overflow: "hidden", flexShrink: 0 }}>{post.title}</h3>

        {/* Excerpt — fills remaining space, clamps */}
        <p style={{ color: "#6b7280", fontSize: 10, fontFamily: FONT, lineHeight: 1.5, margin: "0 0 6px", display: "-webkit-box", WebkitLineClamp: 2, WebkitBoxOrient: "vertical", overflow: "hidden", flex: 1 }}>{post.excerpt}</p>

        {/* Tags row */}
        <div style={{ display: "flex", gap: 4, flexWrap: "nowrap", overflow: "hidden", marginBottom: 8, flexShrink: 0 }}>
          {post.tags.slice(0, 2).map(tag => <span key={tag} style={{ padding: "2px 7px", borderRadius: 99, fontSize: 9, fontWeight: 700, background: "rgba(35,197,94,0.08)", color: "#23c55e", border: "1px solid rgba(35,197,94,0.18)", whiteSpace: "nowrap" }}>#{tag}</span>)}
        </div>
      </div>

      {/* Action bar — pinned to bottom */}
      <div style={{ padding: "6px 13px 8px", borderTop: "1px solid rgba(0,0,0,0.05)", display: "flex", alignItems: "center", gap: 0, flexShrink: 0 }}>
        <button className="action-btn" onClick={() => setLiked(l => !l)} style={{ color: liked ? "#ef4444" : "#9ca3af", fontSize: 11, padding: "4px 7px" }}>
          <svg width="12" height="12" viewBox="0 0 24 24" fill={liked ? "#ef4444" : "none"} stroke="currentColor" strokeWidth="2" strokeLinecap="round"><path d="M20.84 4.61a5.5 5.5 0 0 0-7.78 0L12 5.67l-1.06-1.06a5.5 5.5 0 0 0-7.78 7.78l1.06 1.06L12 21.23l7.78-7.78 1.06-1.06a5.5 5.5 0 0 0 0-7.78z"/></svg>
          {post.likes + (liked ? 1 : 0)}
        </button>
        <button className="action-btn" style={{ color: "#9ca3af", fontSize: 11, padding: "4px 7px" }}>
...
// ── PostView: compact panel, 2-col square grid, independent scroll ────────────
function PostView() {
  const [activeTab, setActiveTab] = useState("All Posts");
  const [savedIds, setSavedIds] = useState(() => new Set(POSTS.filter(p => p.saved).map(p => p.id)));
  const [showAllComm, setShowAllComm] = useState(false);
  const [commTab, setCommTab] = useState("country");

  const toggleSave = (id) => setSavedIds(s => { const n = new Set(s); n.has(id) ? n.delete(id) : n.add(id); return n; });

  const visiblePosts = activeTab === "Saved Posts" ? POSTS.filter(p => savedIds.has(p.id)) : POSTS;
  const joinedAll = Object.values(ALL_COMMUNITIES).flat().filter(c => c.joined);

  return (
    <>
      {showAllComm && <AllCommunitiesModal onClose={() => setShowAllComm(false)} />}

      {/* Two-column layout: narrow post panel + sidebar. Fixed height, each col scrolls independently */}
      <div style={{ display: "grid", gridTemplateColumns: "minmax(0,1fr) 260px", gap: 16, animation: "fade-up .4s ease both", height: "calc(100vh - 108px)", overflow: "hidden" }}>

        {/* ── Left: post feed (square grid) ── */}
        <div style={{ display: "flex", flexDirection: "column", overflow: "hidden", minWidth: 0 }}>

          {/* Fixed header */}
          <div style={{ flexShrink: 0 }}>
            <div style={{ marginBottom: 12 }}>
              <h2 style={{ fontWeight: 800, fontSize: 18, color: G.text, fontFamily: FONT, letterSpacing: -.5, margin: 0 }}>My Posts</h2>
              <p style={{ color: G.textSub, fontSize: 11, fontFamily: FONT, marginTop: 2, marginBottom: 0 }}>Posts you've shared with the community</p>
            </div>
            <div style={{ display: "flex", borderBottom: "2px solid rgba(255,255,255,0.06)", marginBottom: 14 }}>
              {["All Posts", "Saved Posts"].map(tab => (
                <button key={tab} onClick={() => setActiveTab(tab)} style={{ padding: "7px 16px", borderRadius: 0, border: "none", background: "transparent", cursor: "pointer", fontFamily: FONT, fontSize: 12, fontWeight: activeTab === tab ? 700 : 500, color: activeTab === tab ? G.text : G.textSub, borderBottom: activeTab === tab ? `2px solid ${G.mint}` : "2px solid transparent", marginBottom: -2, transition: "all .15s", display: "flex", alignItems: "center", gap: 6 }}>
                  {tab}
                  {tab === "Saved Posts" && savedIds.size > 0 && (
                    <span style={{ padding: "1px 6px", borderRadius: 99, background: "rgba(251,191,36,0.15)", border: "1px solid rgba(251,191,36,0.3)", color: G.gold, fontSize: 9, fontWeight: 800, fontFamily: FONT }}>{savedIds.size}</span>
                  )}
                </button>
              ))}
            </div>
          ...
          {/* Scrollable 2-col square grid */}
          <div style={{ flex: 1, overflowY: "auto", paddingRight: 2 }}>
            {visiblePosts.length === 0 ? (
              <div style={{ textAlign: "center", padding: "50px 20px" }}>
                <div style={{ fontSize: 34, marginBottom: 12 }}>🔖</div>
                <div style={{ fontWeight: 700, fontSize: 14, color: G.text, fontFamily: FONT, marginBottom: 5 }}>No saved posts yet</div>
                <div style={{ color: G.textSub, fontSize: 12, fontFamily: FONT }}>Tap the bookmark icon on any post.</div>
              </div>
            ) : (
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12, paddingBottom: 16 }}>
                {visiblePosts.map((p, i) => (
                  <PostCard key={p.id} post={p} delay={i * 50} savedIds={savedIds} onToggleSave={toggleSave} />
                ))}
              </div>
            )}
          </div>
        ...
        <div style={{ overflowY: "auto", display: "flex", flexDirection: "column", gap: 12 }}>
          ...
          {/* Communities */}
          <div style={{ background: "#1a2537", borderRadius: 16, padding: 16, border: "1px solid rgba(255,255,255,0.07)", animation: "fade-up .4s ease .1s both", flexShrink: 0 }}>
            <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 3 }}>
              <div style={{ fontWeight: 800, fontSize: 13, color: G.text, fontFamily: FONT }}>My Communities</div>
              <span style={{ padding: "2px 7px", borderRadius: 99, background: "rgba(35,197,94,0.1)", border: "1px solid rgba(35,197,94,0.22)", color: G.mint, fontSize: 9, fontWeight: 800, fontFamily: FONT }}>{joinedAll.length} joined</span>
            </div>
            <div style={{ color: G.textSub, fontSize: 10, fontFamily: FONT, marginBottom: 12 }}>Filter by type</div>

            <div style={{ display: "flex", gap: 3, background: "rgba(255,255,255,0.04)", borderRadius: 9, padding: 3, marginBottom: 12, border: "1px solid rgba(255,255,255,0.06)" }}>
              {[{ id: "country", label: "Countries" }, { id: "city", label: "Cities" }, { id: "university", label: "Unis" }].map(t => (
                <button key={t.id} onClick={() => setCommTab(t.id)} style={{ flex: 1, padding: "4px 3px", borderRadius: 6, border: "none", background: commTab === t.id ? "rgba(35,197,94,0.15)" : "transparent", color: commTab === t.id ? G.mint : G.textSub, fontWeight: commTab === t.id ? 700 : 500, fontSize: 9, fontFamily: FONT, cursor: "pointer", transition: "all .15s" }}>{t.label}</button>
              ))}
            </div>

            {ALL_COMMUNITIES[commTab].filter(c => c.joined).map(c => (
              <div key={c.name} className="comm-row" style={{ display: "flex", alignItems: "center", gap: 9, marginBottom: 2, padding: "7px 8px" }}>
                <div style={{ width: 32, height: 32, borderRadius: 9, background: c.bg, display: "flex", alignItems: "center", justifyContent: "center", fontSize: 15, flexShrink: 0 }}>{c.emoji}</div>
                <div style={{ flex: 1, minWidth: 0 }}>
                  <div style={{ fontWeight: 700, fontSize: 11, color: G.text, fontFamily: FONT, whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis" }}>{c.name}</div>
                  <div style={{ color: G.textSub, fontSize: 9, fontFamily: FONT }}>{c.members} members</div>
                </div>
                <div style={{ width: 5, height: 5, borderRadius: "50%", background: G.mint, flexShrink: 0 }} />
              </div>
            ))}
            {ALL_COMMUNITIES[commTab].filter(c => c.joined).length === 0 && (
              <div style={{ color: G.textMuted, fontSize: 11, fontFamily: FONT, padding: "6px 0" }}>No joined communities here.</div>
            )}

            <button onClick={() => setShowAllComm(true)} style={{ marginTop: 10, width: "100%", padding: "7px 0", background: "rgba(35,197,94,0.06)", border: "1px solid rgba(35,197,94,0.18)", borderRadius: 9, color: G.mint, fontWeight: 700, fontSize: 11, fontFamily: FONT, cursor: "pointer", transition: "all .15s", display: "flex", alignItems: "center", justifyContent: "center", gap: 5 }}
              onMouseEnter={e => e.currentTarget.style.background = "rgba(35,197,94,0.12)"}
              onMouseLeave={e => e.currentTarget.style.background = "rgba(35,197,94,0.06)"}>
          ...
          {/* Trending */}
          <div style={{ background: "#1a2537", borderRadius: 16, padding: 16, border: "1px solid rgba(255,255,255,0.07)", animation: "fade-up .4s ease .18s both", flexShrink: 0 }}>
            <div style={{ fontWeight: 800, fontSize: 13, color: G.text, fontFamily: FONT, marginBottom: 3 }}>Top Trending</div>
            <div style={{ color: G.textSub, fontSize: 10, fontFamily: FONT, marginBottom: 12 }}>Most popular destinations</div>
            {TRENDING.map((t, i) => (
              <div key={t.country} className="trend-row" style={{ display: "flex", alignItems: "center", gap: 9, padding: "8px 4px", borderBottom: i < 4 ? "1px solid rgba(255,255,255,0.05)" : "none" }}>
                <span style={{ color: G.textMuted, fontWeight: 700, fontSize: 10, fontFamily: FONT, minWidth: 14 }}>{i+1}</span>
                <span style={{ fontSize: 18 }}>{t.flag}</span>
                <div style={{ flex: 1 }}>
                  <div style={{ fontWeight: 700, fontSize: 12, color: G.text, fontFamily: FONT }}>{t.country}</div>
                  <div style={{ color: G.textSub, fontSize: 9, fontFamily: FONT }}>{t.students} students</div>
                </div>
              </div>
            ))}
          </div>
      ...
      {topTab === "Work" && (
        <div style={{ minHeight: "100vh", background: G.deep, fontFamily: FONT, padding: "22px 26px" }}>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "260px 1fr", gap: 16, alignItems: "start" }}>
            {/* Left sidebar — never covered by modal */}
            ...
            {/*
              Main content wrapper — position:relative is the stacking context
              for both AllJobsModal and AllRequirementsModal overlays.
              overflow:hidden clips the absolute overlays to this column only.
            */}
            <div style={{ position: "relative", overflow: "hidden", minHeight: "100vh" }}>
              <WorkView profile={profile} />
            </div>
          </div>
        </div>
      )}
