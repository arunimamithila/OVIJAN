...
const STYLES = `
  ...
  *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  .icon-btn:hover { color: #23c55e; }

  .tag-pill {
    display: inline-flex; align-items: center; gap: 4px;
    padding: 2px 10px;
    background: rgba(35,197,94,0.09);
    color: #23c55e;
    font-size: 0.71rem;
    border-radius: 20px;
  ...
  useEffect(() => {
    ...
    const resize = () => {
      ...
      for (let i = 0; i < count; i++) {
        dots.push({ x:Math.random()*W, y:Math.random()*H, vx:(Math.random()-0.5)*0.2, vy:(Math.random()-0.5)*0.2, r:Math.random()*1.2+0.4, a:Math.random()*0.35+0.08 });
      }
    };
    const GRID = 80;
    const draw = () => {
      ...
      for(let x=GRID;x<W;x+=GRID) for(let y=GRID;y<H;y+=GRID){ctx.beginPath();ctx.arc(x,y,1.2,0,Math.PI*2);ctx.fill();}
      dots.forEach(d=>{
        d.x+=d.vx; d.y+=d.vy;
...
const Ic = {
  ...
  Heart:    ({f})=><svg width="16" height="16" fill={f?'#23c55e':'none'} viewBox="0 0 24 24" stroke={f?'#23c55e':'currentColor'} strokeWidth="2"><path d="M20.84 4.61a5.5 5.5 0 0 0-7.78 0L12 5.67l-1.06-1.06a5.5 5.5 0 0 0-7.78 7.78l1.06 1.06L12 21.23l7.78-7.78 1.06-1.06a5.5 5.5 0 0 0 0-7.78z"/></svg>,
  ...
  Bookmark: ({f})=><svg width="16" height="16" fill={f?'#23c55e':'none'} viewBox="0 0 24 24" stroke={f?'#23c55e':'currentColor'} strokeWidth="2"><path d="M19 21l-7-5-7 5V5a2 2 0 0 1 2-2h10a2 2 0 0 1 2 2z"/></svg>,
  ...
  Globe:    ()=><svg width="32" height="32" fill="none" viewBox="0 0 24 24" stroke="#23c55e" strokeWidth="1.5"><circle cx="12" cy="12" r="10"/><line x1="2" y1="12" x2="22" y2="12"/><path d="M12 2a15.3 15.3 0 0 1 4 10 15.3 15.3 0 0 1-4 10 15.3 15.3 0 0 1-4-10 15.3 15.3 0 0 1 4-10z"/></svg>,
  ...
  Trend:    ()=><svg width="15" height="15" fill="none" viewBox="0 0 24 24" stroke="#23c55e" strokeWidth="2"><polyline points="23 6 13.5 15.5 8.5 10.5 1 18"/><polyline points="17 6 23 6 23 12"/></svg>,
  Cal:      ()=><svg width="15" height="15" fill="none" viewBox="0 0 24 24" stroke="#23c55e" strokeWidth="2"><rect x="3" y="4" width="18" height="18" rx="2"/><line x1="16" y1="2" x2="16" y2="6"/><line x1="8" y1="2" x2="8" y2="6"/><line x1="3" y1="10" x2="21" y2="10"/></svg>,
  File:     ()=><svg width="15" height="15" fill="none" viewBox="0 0 24 24" stroke="#23c55e" strokeWidth="2"><path d="M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8z"/><polyline points="14 2 14 8 20 8"/><line x1="16" y1="13" x2="8" y2="13"/><line x1="16" y1="17" x2="8" y2="17"/></svg>,
  Users:    ()=><svg width="15" height="15" fill="none" viewBox="0 0 24 24" stroke="#23c55e" strokeWidth="2"><path d="M17 21v-2a4 4 0 0 0-4-4H5a4 4 0 0 0-4 4v2"/><circle cx="9" cy="7" r="4"/><path d="M23 21v-2a4 4 0 0 0-3-3.87"/><path d="M16 3.13a4 4 0 0 1 0 7.75"/></svg>,
  ...
  Check:    ()=><svg width="20" height="20" fill="none" viewBox="0 0 24 24" stroke="#23c55e" strokeWidth="2.5"><polyline points="20 6 9 17 4 12"/></svg>,
  ...
  Shield:   ()=><svg width="15" height="15" fill="none" viewBox="0 0 24 24" stroke="#23c55e" strokeWidth="2"><path d="M12 22s8-4 8-10V5l-8-3-8 3v7c0 6 8 10 8 10z"/></svg>,
  Award:    ()=><svg width="15" height="15" fill="none" viewBox="0 0 24 24" stroke="#23c55e" strokeWidth="2"><circle cx="12" cy="8" r="6"/><path d="M15.477 12.89L17 22l-5-3-5 3 1.523-9.11"/></svg>,
...
const Avatar = ({ initials, size = 42 }) => (
  <div style={{ width:size, height:size, borderRadius:'50%', flexShrink:0, background:'#23c55e', display:'flex', alignItems:'center', justifyContent:'center', color:'#fff', fontWeight:700, fontSize:size>36?'0.82rem':'0.7rem' }}>
    {initials}
  </div>
);
    ...
    <div style={OVERLAY} onClick={e => e.target === e.currentTarget && onClose()}>
      <div className="dark-card modal-in" style={{ width:'100%', maxWidth, maxHeight:'88vh', display:'flex', flexDirection:'column', overflow:'hidden' }}>
        <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between', padding:'16px 20px', borderBottom:'1px solid rgba(255,255,255,0.07)', flexShrink:0 }}>
          <div>
            <p style={{ fontWeight:700, fontSize:'0.95rem', color:'#e2e8f0' }}>{title}</p>
            {subtitle && <p style={{ fontSize:'0.72rem', color:'#4b6080', marginTop:2 }}>{subtitle}</p>}
          </div>
          <button className="icon-btn" onClick={onClose} style={{ width:30, height:30, borderRadius:8, border:'1px solid rgba(255,255,255,0.1)', color:'#64748b' }}>
            <Ic.X />
          </button>
        </div>
    ...
    <ModalShell title="Upcoming Events" subtitle={`${ALL_EVENTS.length} events scheduled`} onClose={onClose} maxWidth={580}>
      <div style={{ display:'flex', flexDirection:'column', gap:12 }}>
        {ALL_EVENTS.map((ev, i) => {
          ...
          const color   = typeColors[ev.type] || '#23c55e';
          return (
            <div key={i} className="fade-up"
              style={{ padding:16, border:'1px solid rgba(255,255,255,0.08)', borderRadius:14, transition:'border-color 0.18s', animationDelay:`${i*50}ms` }}
              onMouseOver={e => e.currentTarget.style.borderColor='rgba(35,197,94,0.35)'}
              ...
              <div style={{ display:'flex', alignItems:'flex-start', justifyContent:'space-between', gap:12 }}>
                <div style={{ flex:1, minWidth:0 }}>
                  <div style={{ display:'flex', alignItems:'center', gap:8, marginBottom:6, flexWrap:'wrap' }}>
                    ...
                    <span style={{ fontSize:'0.69rem', color:'#4b6080', display:'flex', alignItems:'center', gap:4 }}><Ic.Clock/>{ev.date} · {ev.time}</span>
                  </div>
                  <p style={{ fontWeight:700, fontSize:'0.86rem', color:'#e2e8f0', marginBottom:4 }}>{ev.title}</p>
                  <p style={{ fontSize:'0.78rem', color:'#64748b', lineHeight:1.6, marginBottom:8 }}>{ev.desc}</p>
                  <div style={{ display:'flex', alignItems:'center', gap:12, fontSize:'0.72rem', color:'#4b6080' }}>
                    <span>Host: <span style={{ color:'#94a3b8' }}>{ev.host}</span></span>
                ...
                <button
                  ...
                  style={{ flexShrink:0, padding:'7px 14px', borderRadius:10, fontSize:'0.76rem', display:'flex', alignItems:'center', gap:6, background:isReg?'rgba(35,197,94,0.15)':undefined, color:isReg?'#23c55e':undefined, border:isReg?'1px solid rgba(35,197,94,0.3)':undefined }}>
                  {isLoading
...
function GuidelinesModal({ onClose }) {
  return (
    <ModalShell title="Community Guidelines" subtitle="Last updated May 2026 · Applies to all members" onClose={onClose} maxWidth={560}>
      <div style={{ marginBottom:14, padding:12, borderRadius:12, background:'rgba(35,197,94,0.07)', border:'1px solid rgba(35,197,94,0.2)' }}>
        <p style={{ fontSize:'0.8rem', color:'#4ade80', lineHeight:1.7 }}>
      ...
      <div style={{ display:'flex', flexDirection:'column', gap:10 }}>
        {FULL_GUIDELINES.map((g, i) => (
          <div key={i} className="fade-up"
            style={{ padding:14, borderRadius:13, border:'1px solid rgba(255,255,255,0.07)', background:'rgba(255,255,255,0.02)', animationDelay:`${i*40}ms` }}>
            <div style={{ display:'flex', alignItems:'flex-start', gap:12 }}>
              <span style={{ fontSize:'1.3rem', flexShrink:0, marginTop:1 }}>{g.icon}</span>
              <div>
                <p style={{ fontWeight:700, fontSize:'0.84rem', color:'#e2e8f0', marginBottom:4 }}>{g.title}</p>
                <p style={{ fontSize:'0.78rem', color:'#64748b', lineHeight:1.65 }}>{g.body}</p>
              </div>
            </div>
          </div>
        ))}
      ...
      <div style={{ marginTop:16, padding:14, borderRadius:12, border:'1px solid rgba(239,68,68,0.2)', background:'rgba(239,68,68,0.04)' }}>
        <p style={{ fontSize:'0.77rem', color:'#f87171', lineHeight:1.65 }}>
          <strong>Questions?</strong> Reach the moderation team at <span style={{ color:'#23c55e' }}>mods@communityname.com</span> or use the in-app Report button on any post.
        </p>
      </div>
    </ModalShell>
  );
    ...
    <ModalShell title="Top Contributors" subtitle={`${ALL_CONTRIBUTORS.length} members · Ranked by community points`} onClose={onClose} maxWidth={540}>
      <div style={{ marginBottom:14 }}>
        <input
          ...
          style={{ width:'100%', padding:'9px 13px', background:'rgba(255,255,255,0.05)', border:'1px solid rgba(255,255,255,0.1)', borderRadius:12, color:'#e2e8f0', fontSize:'0.82rem', outline:'none', fontFamily:'Sora, sans-serif' }}
        />
      </div>
      <div style={{ display:'flex', flexDirection:'column', gap:10 }}>
        {filtered.map((u, i) => {
          ...
          return (
            <div key={i} className="fade-up"
              style={{ display:'flex', alignItems:'center', justifyContent:'space-between', gap:12, padding:'12px 14px', borderRadius:13, border:'1px solid rgba(255,255,255,0.07)', background:'rgba(255,255,255,0.02)', animationDelay:`${i*40}ms` }}>
              <div style={{ display:'flex', alignItems:'center', gap:12, flex:1, minWidth:0 }}>
                <div style={{ position:'relative', flexShrink:0 }}>
                  <Avatar initials={u.name.split(' ').map(n=>n[0]).join('')} size={40}/>
                  {u.online && <span style={{ position:'absolute', bottom:0, right:0, width:9, height:9, background:'#23c55e', borderRadius:'50%', border:'2px solid #131c2e', animation:'pulse-dot 2s ease infinite' }}/>}
                  {i < 3 && (
                ...
                <div style={{ flex:1, minWidth:0 }}>
                  <div style={{ display:'flex', alignItems:'center', gap:6 }}>
                    <p style={{ fontWeight:700, fontSize:'0.84rem', color:'#e2e8f0' }}>{u.name}</p>
                    <span style={{ fontSize:'0.68rem', color:'#4b6080' }}>{u.flag} {u.country}</span>
                  </div>
                  <p style={{ fontSize:'0.74rem', color:'#64748b', lineHeight:1.5, marginTop:2 }}>{u.bio}</p>
                  <div style={{ display:'flex', gap:12, marginTop:4, fontSize:'0.69rem', color:'#4b6080' }}>
                    <span style={{ color:'#23c55e', fontWeight:700 }}>{u.pts} pts</span>
                    <span>{u.posts} posts</span>
                    <span>{u.online ? <span style={{ color:'#4ade80' }}>● Online</span> : 'Offline'}</span>
                  </div>
                </div>
              </div>
              <button
                onClick={() => onToggleFollow(u.name)}
                style={{ flexShrink:0, padding:'6px 14px', borderRadius:9, fontSize:'0.72rem', fontWeight:700, cursor:'pointer', transition:'all .15s', border:`1px solid ${followed?'rgba(35,197,94,.35)':'rgba(35,197,94,.5)'}`, background:followed?'rgba(35,197,94,.1)':'#23c55e', color:followed?'#23c55e':'#fff', fontFamily:'inherit' }}>
        ...
        {filtered.length === 0 && (
          <p style={{ textAlign:'center', color:'#4b6080', fontSize:'0.84rem', padding:24 }}>No contributors match your search.</p>
        )}
      </div>
    </ModalShell>
  );
      ...
      <div className="dark-card modal-in" style={{ width:'100%', maxWidth:440, overflow:'hidden' }}>
        <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between', padding:'16px 20px', borderBottom:'1px solid rgba(255,255,255,0.07)' }}>
          <div style={{ display:'flex', alignItems:'center', gap:10 }}>
            ...
            <div>
              <p style={{ fontWeight:700, fontSize:'0.93rem', color:'#e2e8f0' }}>Join {country.name}</p>
              <p style={{ fontSize:'0.72rem', color:'#4b6080' }}>{country.members} members · Open community</p>
            </div>
          </div>
          {!sent && (
            <button className="icon-btn" onClick={onClose} style={{ width:30, height:30, borderRadius:8, border:'1px solid rgba(255,255,255,0.1)', color:'#64748b' }}>
        ...
        {sent ? (
          <div style={{ padding:'36px 20px', textAlign:'center' }}>
            <div style={{ width:52, height:52, borderRadius:'50%', background:'rgba(35,197,94,0.12)', border:'1px solid rgba(35,197,94,0.35)', display:'flex', alignItems:'center', justifyContent:'center', margin:'0 auto 14px' }}>
              <Ic.Check />
            </div>
            <p style={{ fontWeight:700, fontSize:'0.95rem', color:'#e2e8f0', marginBottom:6 }}>Request Sent!</p>
            <p style={{ fontSize:'0.82rem', color:'#94a3b8' }}>Your join request for {country.name} has been submitted.</p>
          </div>
        ) : (
          <div style={{ padding:'18px 20px' }}>
            <div style={{ padding:12, borderRadius:12, background:'rgba(35,197,94,0.08)', border:'1px solid rgba(35,197,94,0.22)', marginBottom:16 }}>
              <p style={{ fontSize:'0.8rem', color:'#4ade80', lineHeight:1.6 }}>📌 Most communities approve within 24 hours.</p>
            </div>
            <label style={{ display:'block', fontSize:'0.75rem', color:'#94a3b8', marginBottom:6 }}>Introduce yourself (optional)</label>
            <textarea
              value={msg}
              onChange={e => setMsg(e.target.value)}
              placeholder={`Tell the ${country.name} community about yourself...`}
              style={{ width:'100%', minHeight:90, padding:'10px 13px', background:'rgba(255,255,255,0.05)', border:'1px solid rgba(255,255,255,0.1)', borderRadius:12, color:'#e2e8f0', fontSize:'0.85rem', resize:'none', outline:'none', lineHeight:1.6, marginBottom:14, fontFamily:'inherit' }}
            />
            <div style={{ display:'flex', gap:10 }}>
              <button onClick={onClose}
                style={{ flex:1, padding:10, borderRadius:12, fontWeight:600, fontSize:'0.85rem', background:'rgba(255,255,255,0.05)', border:'1px solid rgba(255,255,255,0.12)', color:'#94a3b8', cursor:'pointer', fontFamily:'inherit' }}>
                Cancel
              </button>
    ...
    <>
      <div className="dark-card" style={{ padding:16, marginBottom:16 }}>
        <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom:12 }}>
          <h2 style={{ fontSize:'0.97rem', fontWeight:700, color:'#e2e8f0', letterSpacing:'-0.01em' }}>Explore Communities</h2>
          <button onClick={onViewAll} style={{ background:'none', border:'none', cursor:'pointer', fontSize:'0.78rem', color:'#23c55e', fontFamily:'inherit', fontWeight:600 }}>
        ...
        {viewAll ? (
          <div style={{ display:'grid', gridTemplateColumns:'repeat(auto-fill,minmax(180px,1fr))', gap:10 }}>
            {COUNTRIES.map((c, i) => {
              ...
              return (
                <div key={i}
                  style={{ borderRadius:14, overflow:'hidden', position:'relative', cursor:'pointer', height:140, border:'1px solid rgba(255,255,255,0.08)', transition:'transform .2s' }}
                ...
                >
                  ...
                  <div style={{ position:'absolute', inset:0, background:'linear-gradient(to top,rgba(11,17,32,.9) 0%,rgba(11,17,32,.2) 55%,transparent)' }}/>
                  ...
                  <div style={{ position:'absolute', bottom:10, left:12 }}>
                    ...
                    <div style={{ fontSize:'0.68rem', color:'rgba(35,197,94,0.85)' }}>👥 {c.members} members</div>
                  </div>
                </div>
              );
            })}
          ...
          >
            <button className="carr-arrow" onClick={() => scroll('l')}
              style={{ position:'absolute', left:0, top:'50%', transform:'translateY(-50%)', zIndex:5, width:32, height:32, borderRadius:'50%', background:'rgba(255,255,255,0.08)', border:'1px solid rgba(255,255,255,0.14)', color:'#23c55e', cursor:'pointer', display:'flex', alignItems:'center', justifyContent:'center' }}>
              <Ic.ChevL />
            </button>
            ...
            <div id="c-carousel" className="scrollbar-hide" style={{ display:'flex', gap:10, overflowX:'auto', paddingBottom:2 }}>
              {displayCountries.map((c, i) => {
                ...
                return (
                  <div key={i}
                    style={{ flexShrink:0, width:200, height:130, borderRadius:14, overflow:'hidden', position:'relative', cursor:'pointer', transition:'transform 0.25s cubic-bezier(0.22,1,0.36,1)', border:'1px solid rgba(255,255,255,0.08)' }}
                  ...
                  >
                    <img src={c.image} alt={c.name} style={{ width:'100%', height:'100%', objectFit:'cover' }} loading="lazy"/>
                    <div style={{ position:'absolute', inset:0, background:'linear-gradient(to top, rgba(11,17,32,0.9) 0%, rgba(11,17,32,0.2) 55%, transparent)' }}/>
                    <button className="jbtn accent-btn"
                      onClick={e => { e.stopPropagation(); if (!j) setJoinTarget(c); }}
              ...
              <div
                onClick={onViewAll}
                style={{ flexShrink:0, width:200, height:130, display:'flex', flexDirection:'column', alignItems:'center', justifyContent:'center', gap:8, borderRadius:14, cursor:'pointer', background:'rgba(255,255,255,0.03)', border:'1px dashed rgba(35,197,94,0.25)', transition:'background 0.18s, border-color 0.18s' }}
                onMouseOver={e => { e.currentTarget.style.background='rgba(35,197,94,0.06)'; e.currentTarget.style.borderColor='rgba(35,197,94,0.45)'; }}
                onMouseOut={e => { e.currentTarget.style.background='rgba(255,255,255,0.03)'; e.currentTarget.style.borderColor='rgba(35,197,94,0.25)'; }}
              >
                <div className="globe-spin"><Ic.Globe /></div>
                <span style={{ fontWeight:700, color:'#fff', fontSize:'0.82rem' }}>View All</span>
              </div>
            </div>

            <button className="carr-arrow" onClick={() => scroll('r')}
              style={{ position:'absolute', right:0, top:'50%', transform:'translateY(-50%)', zIndex:5, width:32, height:32, borderRadius:'50%', background:'rgba(255,255,255,0.08)', border:'1px solid rgba(255,255,255,0.14)', color:'#23c55e', cursor:'pointer', display:'flex', alignItems:'center', justifyContent:'center' }}>
              <Ic.ChevR />
            </button>
          </div>
        )}
      ...
      {tabs.map(t => {
        ...
        return (
          <button key={t} onClick={() => setActive(t)}
            style={{ flex:1, padding:'8px 4px', borderRadius:10, border:on?'1px solid rgba(35,197,94,0.45)':'1px solid transparent', background:on?'rgba(35,197,94,0.14)':'transparent', color:on?'#23c55e':'#64748b', fontFamily:'inherit', fontWeight:600, fontSize:'0.78rem', cursor:'pointer', transition:'all 0.15s' }}>
            {t}
          </button>
        );
      })}
...
function TrendingBanner({ trendingCount }) {
  return (
    <div className="dark-card" style={{ padding:16, marginBottom:14, border:'1px solid rgba(35,197,94,0.2)', background:'rgba(35,197,94,0.04)' }}>
      <div style={{ display:'flex', alignItems:'center', gap:8, marginBottom:12 }}>
        <Ic.Trend />
        <span style={{ color:'#e2e8f0', fontWeight:700, fontSize:'0.86rem' }}>Trending Now</span>
        <span style={{ padding:'2px 8px', background:'rgba(35,197,94,0.15)', border:'1px solid rgba(35,197,94,0.3)', borderRadius:20, fontSize:'0.67rem', color:'#23c55e', fontWeight:700 }}>{trendingCount} posts</span>
      </div>
      <div style={{ display:'flex', gap:8, flexWrap:'wrap' }}>
        {['Scholarship','VisaGuide','Oxford','DAAD','USA','StudyAbroad'].map(t => (
          <span key={t} style={{ padding:'4px 12px', borderRadius:20, fontSize:'0.73rem', fontWeight:700, cursor:'pointer', background:'rgba(35,197,94,0.1)', border:'1px solid rgba(35,197,94,0.25)', color:'#23c55e' }}>#{t}</span>
        ))}
      </div>
    </div>
  );
...
function NewBanner({ count, onBack }) {
  return (
    <div className="dark-card" style={{ padding:'14px 16px', marginBottom:14, display:'flex', alignItems:'center', gap:12 }}>
      <div style={{ width:8, height:8, borderRadius:'50%', background:'#23c55e', animation:'pulse-dot 2s ease infinite', flexShrink:0 }}/>
      <span style={{ color:'#94a3b8', fontSize:'0.82rem' }}>Showing <strong style={{ color:'#e2e8f0' }}>{count} new posts</strong> from the last 24 hours</span>
      <button onClick={onBack} style={{ marginLeft:'auto', background:'none', border:'none', cursor:'pointer', color:'#23c55e', fontFamily:'inherit', fontSize:'0.76rem', fontWeight:600 }}>Back to feed</button>
    </div>
  );
}
  ...
  return (
    <div className="dark-card" style={{ padding:'14px 16px', marginBottom:14 }}>
      <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between' }}>
        <div>
          <p style={{ color:'#e2e8f0', fontWeight:700, fontSize:'0.86rem', marginBottom:2 }}>Following {followingCount} people</p>
          <p style={{ color:'#64748b', fontSize:'0.75rem' }}>Posts from people you follow appear here</p>
        </div>
        <div style={{ display:'flex' }}>
          {followingNames.slice(0,3).map((n,i) => (
            <div key={i} style={{ width:28, height:28, borderRadius:'50%', background:'#23c55e', border:'2px solid #0b1120', display:'flex', alignItems:'center', justifyContent:'center', color:'#fff', fontWeight:700, fontSize:'0.65rem', marginLeft: i > 0 ? -6 : 0 }}>
              {n[0]}
            </div>
          ))}
        </div>
      ...
      {postCount === 0 && (
        <div style={{ marginTop:12, padding:14, background:'rgba(35,197,94,0.06)', borderRadius:10, border:'1px solid rgba(35,197,94,0.15)', textAlign:'center' }}>
          <p style={{ color:'#94a3b8', fontSize:'0.82rem' }}>No posts yet from people you follow.<br/>Discover and follow more people in the sidebar!</p>
        </div>
      )}
    </div>
  );
    ...
    <div className="dark-card" style={{ padding:16, marginBottom:14 }}>
      <div style={{ display:'flex', alignItems:'center', gap:12 }}>
        ...
        <button onClick={onOpen}
          style={{ flex:1, textAlign:'left', padding:'10px 16px', background:'rgba(255,255,255,0.05)', border:'1px solid rgba(255,255,255,0.1)', borderRadius:24, color:'#4b6080', fontFamily:'inherit', fontSize:'0.85rem', cursor:'pointer', transition:'border-color 0.18s' }}
          onMouseOver={e => e.currentTarget.style.borderColor='rgba(35,197,94,0.5)'}
      ...
      <div style={{ display:'flex', gap:4, marginTop:12, paddingTop:12, borderTop:'1px solid rgba(255,255,255,0.07)' }}>
        {[{icon:<Ic.Image/>,label:'Photo'},{icon:<Ic.Hash/>,label:'Tag'},{icon:<Ic.Pin/>,label:'Location'}].map((item,i) => (
          <button key={i} onClick={onOpen} className="icon-btn"
            ...
            onMouseOver={e => { e.currentTarget.style.background='rgba(35,197,94,0.08)'; e.currentTarget.style.color='#23c55e'; }}
...
function PostCard({ post, delay=0, isFollowing, onToggleFollow }) {
  ...
  return (
    <div className="card fade-up"
      ...
      onMouseOver={e => e.currentTarget.style.boxShadow='0 4px 20px rgba(35,197,94,0.1)'}
      onMouseOut={e => e.currentTarget.style.boxShadow='0 1px 8px rgba(0,0,0,0.06)'}>

      {toast && (
        <div style={{ position:'absolute', top:14, right:14, background:'#23c55e', color:'#fff', padding:'5px 13px', borderRadius:10, fontSize:'0.76rem', fontWeight:600, zIndex:5 }}>
          {toast}
        </div>
      )}
        ...
        <div style={{ display:'flex', alignItems:'center', gap:8, position:'relative' }}>
          {!isDefaultFollowing && (
            <button onClick={() => onToggleFollow(post.author.name)}
              style={{ padding:'4px 12px', borderRadius:8, fontSize:'0.72rem', fontWeight:700, cursor:'pointer', transition:'all .15s', border:`1px solid ${isFollowing?'rgba(35,197,94,.35)':'rgba(0,0,0,.1)'}`, background:isFollowing?'rgba(35,197,94,.1)':'transparent', color:isFollowing?'#23c55e':'#64748b', fontFamily:'inherit' }}>
      ...
      {post.tags && (
        <div style={{ display:'flex', flexWrap:'wrap', gap:5, marginBottom:10 }}>
          {post.tags.map(tag => <span key={tag} style={{ color:'#23c55e', fontSize:'0.78rem', cursor:'pointer', fontWeight:600 }}>#{tag}</span>)}
        </div>
      )}
        ...
        <div style={{ display:'flex', gap:18 }}>
          {[
            ...
            <button key={i} onClick={btn.fn} className="icon-btn"
              style={{ display:'flex', alignItems:'center', gap:5, fontSize:'0.79rem', fontWeight:600, color:btn.active?'#23c55e':'var(--text-mid)' }}>
              {btn.icon} {btn.label}
            </button>
          ))}
        </div>
        <button onClick={bookmark} className="icon-btn" style={{ color:saved?'#23c55e':'var(--text-mid)' }}>
          <Ic.Bookmark f={saved}/>
        </button>
      </div>
...
function Sidebar({ followedUsers, onToggleFollow, onSeeAllTrending, onSeeAllFollowing, onOpenEvents, onOpenGuidelines, onOpenContributors }) {
  ...
  const SectionTitle = ({ icon, label, onMore }) => (
    <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between', marginBottom:14 }}>
      <div style={{ display:'flex', alignItems:'center', gap:7, color:'#e2e8f0', fontWeight:700, fontSize:'0.86rem' }}>{icon}{label}</div>
      {onMore && <button className="icon-btn" style={{ fontSize:'0.74rem', color:'#23c55e' }} onClick={onMore}>See All</button>}
    </div>
  );
    ...
    <div style={{ display:'flex', flexDirection:'column', gap:12 }}>
      ...
      <div className="dark-card" style={{ padding:16 }}>
        ...
        {trending.map((p,i) => (
          <div key={i}
            ...
            onMouseOut={e => e.currentTarget.style.background='transparent'}>
            <p className="line-clamp-2" style={{ fontSize:'0.79rem', color:'#cbd5e1', marginBottom:3, lineHeight:1.5 }}>{p.title}</p>
            <div style={{ display:'flex', justifyContent:'space-between', fontSize:'0.69rem' }}>
              <span style={{ color:'#4b6080' }}>{p.author}</span>
              <span style={{ color:'#23c55e', fontWeight:600 }}>{p.views} views</span>
            </div>
          </div>
        ))}
      </div>
            ...
            <div key={i} style={{ display:'flex', alignItems:'center', justifyContent:'space-between', marginBottom:12 }}>
              <div style={{ display:'flex', alignItems:'center', gap:10 }}>
                <div style={{ position:'relative' }}>
                  ...
                  {u.online && <span style={{ position:'absolute', bottom:0, right:0, width:8, height:8, background:'#23c55e', borderRadius:'50%', border:'2px solid #131c2e', animation:'pulse-dot 2s ease infinite' }}/>}
                </div>
                <div>
                  <p style={{ fontWeight:600, fontSize:'0.79rem', color:'#e2e8f0' }}>{u.name}</p>
                  <p style={{ fontSize:'0.67rem', color:'#4b6080' }}>{u.flag} {u.country} · <span style={{ color:'#23c55e', fontWeight:600 }}>{u.pts} pts</span></p>
                </div>
              </div>
              <button onClick={() => onToggleFollow(u.name)}
                style={{ padding:'4px 12px', borderRadius:8, fontSize:'0.7rem', fontWeight:700, cursor:'pointer', transition:'all .15s', border:`1px solid ${followed?'rgba(35,197,94,.35)':'rgba(35,197,94,.5)'}`, background:followed?'rgba(35,197,94,.1)':'#23c55e', color:followed?'#23c55e':'#fff', fontFamily:'inherit' }}>
                {followed ? '✓ Following' : 'Follow'}
              </button>
            </div>
          );
      ...
      <div className="dark-card" style={{ padding:16 }}>
        <div style={{ display:'flex', alignItems:'center', gap:7, marginBottom:12 }}>
          <Ic.UserPlus/><span style={{ color:'#e2e8f0', fontWeight:700, fontSize:'0.86rem' }}>Find People</span>
        </div>
        <input type="text" placeholder="Search users…"
          style={{ width:'100%', padding:'8px 12px', background:'rgba(255,255,255,0.05)', border:'1px solid rgba(255,255,255,0.1)', borderRadius:10, color:'#e2e8f0', fontSize:'0.79rem', outline:'none', marginBottom:10, fontFamily:'inherit' }}/>
        <p style={{ fontSize:'0.69rem', color:'#4b6080', marginBottom:8 }}>Suggested Connections</p>
          ...
          return (
            <div key={i} style={{ display:'flex', alignItems:'center', justifyContent:'space-between', padding:'5px 0' }}>
              <div style={{ display:'flex', alignItems:'center', gap:8 }}>
                ...
                <span style={{ fontSize:'0.79rem', color:'#e2e8f0' }}>{name}</span>
              </div>
              <button onClick={() => { onToggleFollow(name); onSeeAllFollowing(); }}
                style={{ width:26, height:26, borderRadius:7, display:'flex', alignItems:'center', justifyContent:'center', padding:0, background:followed?'rgba(35,197,94,.1)':'#23c55e', border:followed?'1px solid rgba(35,197,94,.3)':'none', cursor:'pointer', color:followed?'#23c55e':'#fff', fontSize:'0.75rem', fontWeight:700 }}>
                {followed ? '✓' : '+'}
              </button>
            </div>
          );
      ...
      <div className="dark-card" style={{ padding:16 }}>
        ...
        {events.map((ev,i) => (
          <div key={i}
            style={{ padding:12, border:'1px solid rgba(255,255,255,0.08)', borderRadius:12, marginBottom:8, transition:'border-color 0.15s' }}
            onMouseOver={e => e.currentTarget.style.borderColor='rgba(35,197,94,0.4)'}
            onMouseOut={e => e.currentTarget.style.borderColor='rgba(255,255,255,0.08)'}>
            <span style={{ display:'inline-block', padding:'2px 9px', background:'rgba(35,197,94,0.12)', color:'#23c55e', fontSize:'0.67rem', borderRadius:20, marginBottom:6, fontWeight:700 }}>{ev.type}</span>
            <p style={{ fontWeight:700, fontSize:'0.79rem', marginBottom:2, color:'#e2e8f0' }}>{ev.title}</p>
            <p style={{ fontSize:'0.69rem', color:'#4b6080', marginBottom:8 }}>{ev.date} · {ev.time}</p>
            <button className="accent-btn" style={{ width:'100%', padding:'7px', borderRadius:8, fontSize:'0.76rem' }}>Register</button>
          </div>
        ))}
        <button
          onClick={onOpenEvents}
          style={{ width:'100%', padding:'8px', borderRadius:10, fontSize:'0.76rem', fontWeight:600, cursor:'pointer', background:'rgba(35,197,94,0.07)', border:'1px solid rgba(35,197,94,0.2)', color:'#23c55e', fontFamily:'inherit', marginTop:2 }}>
      ...
      <div className="dark-card" style={{ padding:16 }}>
        <div style={{ display:'flex', alignItems:'center', gap:7, marginBottom:12 }}>
          <Ic.Shield/><span style={{ color:'#e2e8f0', fontWeight:700, fontSize:'0.86rem' }}>Community Guidelines</span>
        </div>
        <ul style={{ listStyle:'none', display:'flex', flexDirection:'column', gap:8 }}>
          {FULL_GUIDELINES.slice(0,3).map((g,i) => (
            <li key={i} style={{ display:'flex', gap:7, fontSize:'0.79rem', color:'#94a3b8' }}>
              <span style={{ color:'#23c55e', marginTop:1 }}>›</span><span>{g.title}</span>
            </li>
          ))}
        </ul>
        <button
          onClick={onOpenGuidelines}
          style={{ marginTop:12, background:'none', border:'none', cursor:'pointer', fontSize:'0.74rem', color:'#23c55e', fontFamily:'inherit', fontWeight:600, padding:0 }}>
...
function CreatePostModal({ isOpen, onClose }) {
  ...
  const labelStyle = { display:'flex', alignItems:'center', gap:6, fontSize:'0.74rem', color:'#64748b', marginBottom:6 };
  ...
  return (
    <div style={OVERLAY} onClick={e => e.target === e.currentTarget && onClose()}>
      <div className="dark-card modal-in" style={{ width:'100%', maxWidth:540, maxHeight:'90vh', display:'flex', flexDirection:'column', overflow:'hidden' }}>
        <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between', padding:'15px 20px', borderBottom:'1px solid rgba(255,255,255,0.07)', flexShrink:0 }}>
          <h2 style={{ fontWeight:700, fontSize:'0.95rem', color:'#e2e8f0' }}>Create Post</h2>
          <button onClick={onClose} className="icon-btn" style={{ width:30, height:30, borderRadius:8, border:'1px solid rgba(255,255,255,0.1)', color:'#64748b' }}>
            <Ic.X />
          </button>
        </div>
        ...
        <div style={{ flex:1, overflowY:'auto', padding:'16px 20px' }}>
          <div style={{ display:'flex', alignItems:'center', gap:11, marginBottom:14 }}>
            <Avatar initials="JD" />
            <p style={{ fontWeight:700, fontSize:'0.87rem', color:'#e2e8f0' }}>John Doe</p>
          </div>

          <textarea value={content} onChange={e => setContent(e.target.value)}
            placeholder="What's on your mind? Share your study abroad experience, tips, or questions…"
            autoFocus
            style={{ width:'100%', minHeight:160, background:'transparent', border:'none', color:'#e2e8f0', fontFamily:'inherit', fontSize:'0.92rem', resize:'none', outline:'none', lineHeight:1.75, marginBottom:12 }}
          />

          {imgSrc && (
            <div style={{ position:'relative', borderRadius:12, overflow:'hidden', marginBottom:12, border:'1px solid rgba(255,255,255,0.1)' }}>
          ...
          <div style={{ marginBottom:14 }}>
            ...
            <select value={country} onChange={e => setCountry(e.target.value)} style={{ ...inputStyle, cursor:'pointer', appearance:'none' }}>
              <option value="" style={{ background:'#131c2e', color:'#94a3b8' }}>Select a country…</option>
              {countries.map(c => <option key={c.name} value={c.name} style={{ background:'#131c2e', color:'#e2e8f0' }}>{c.flag} {c.name}</option>)}
            </select>
          </div>
        ...
        <div style={{ borderTop:'1px solid rgba(255,255,255,0.07)', padding:'12px 20px', flexShrink:0 }}>
          <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between' }}>
            <div style={{ display:'flex', gap:6 }}>
              <label className="icon-btn" style={{ width:32, height:32, borderRadius:8, border:'1px solid rgba(255,255,255,0.1)', cursor:'pointer', color:'#64748b' }}>
                <input type="file" accept="image/*" onChange={uploadImg} style={{ display:'none' }}/>
                <Ic.Image/>
              </label>
            </div>
            <div style={{ display:'flex', gap:10, alignItems:'center' }}>
              <span style={{ fontSize:'0.7rem', color:'#4b6080', fontFamily:'DM Mono, monospace' }}>{content.length} chars</span>
              <button onClick={onClose}
                style={{ padding:'8px 16px', borderRadius:12, fontWeight:600, fontSize:'0.84rem', background:'rgba(255,255,255,0.05)', border:'1px solid rgba(255,255,255,0.12)', color:'#94a3b8', cursor:'pointer', fontFamily:'inherit' }}>
                Cancel
              </button>
            ...
            <div style={{ minWidth:0 }}>
              ...
              {feedPosts.length > 0
                ...
                : (
                  <div className="dark-card" style={{ padding:40, textAlign:'center' }}>
                    <p style={{ color:'#64748b', fontSize:'0.9rem', marginBottom:16 }}>No posts from people you follow yet.</p>
              ...
              <div style={{ display:'flex', flexDirection:'column', alignItems:'center', paddingTop:16, gap:10 }}>
                {hasMore ? (
                  ...
                  <p style={{ color:'#4b6080', fontSize:'0.78rem', fontWeight:600 }}>✓ You're all caught up</p>
                ) : null}
              </div>
            </div>
